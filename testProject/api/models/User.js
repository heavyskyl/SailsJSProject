...
module.exports = {
    ...
    attributes: {
        ...
        toJSON: function () {
            ...
            _.forEach(object.avatar, function (val, key) {
                object.avatar[key] = AmazonS3Service.getAvatarUrl(object, key);
            });
    ...
    afterCreate: function (user, next) {
        AmazonS3Service.putObjects(user.avatar, user.id, function (err, files) {
            AvatarService.removeFiles(files, function (err) {
                next(err);
            });
        });
    },
    ...
    beforeCreate: function (attrs, next) {
        AvatarService.createImages(attrs.avatar, function(err, avatar) {
            if (err) {
                next(err);
            } else {
                attrs.avatar = avatar;
    ...
    beforeValidation: function (attrs, next) {
        ...
        function goNext() {
            if (Object.getOwnPropertyNames(errors).length > 0) {
                ...
                attrs.avatar = JSON.parse(attrs.avatar);
                next();
            }
        }
